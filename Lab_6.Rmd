...
In Lab 5 last week, we discussed hypothesis testing as a framework for making statistical inferences about population parameters using sample data. For example, we discussed the use of the two-sample t-test for comparing means across two independent groups. In Lab 6, we will extend our hypothesis testing approach to situations where we need to compare more than two groups. This is where Analysis of Variance (ANOVA) comes in.

ANOVA, or Analysis of Variance, is a statistical method used to determine whether there are statistically significant differences between the means of three or more independent groups. Instead of performing multiple t-tests (which would increase the risk of type 1 errors), ANOVA allows us to test for differences in means across several groups with a single hypothesis test. ANOVA essentially allows us to investigate whether there is a significant relationship between a categorical independent variable (i.e., the "exposure") and a continuous dependent variable (i.e., "outcome" of interest).
...
2. Next, save the **modified dataset** you created in Lab 2 – called "frmgham2_p1_impute.csv" – in the same folder as your Lab 6 R Markdown file. This is the same file we used in Labs 4-5.
...
Use the `read.csv()` function to import the modified dataset you created in Lab 2. For this to work, the "frmgham2_p1_impute.csv" file will need to be saved in the working directory you specified in the above step. Again, this is the same data file you started with in Labs 4-5.
...
* `bmi_summary <-`: Stores the result of the operation as an object called `bmi_summary`.
...
Before conducting statistical analyses, it’s important to identify outliers — values that are unusually high or low compared to the rest of the data. Outliers can impact assumptions for hypothesis testing, such as normality and homogeneity of variance (more on these later) in ANOVA.
...
The normality assumption in ANOVA requires that the dependent variable (in this case, BMI) is approximately normally distributed within each group (in this case, attained education level). While ANOVA is somewhat robust to violations of normality, especially when sample sizes are large, it is still good practice to check for normality before proceeding.
...
Histograms are a useful way to visually assess whether deviations from normality are meaningful. If the histograms show severe skewness or extreme outliers, we may need to consider transformations or alternative statistical methods. When examining histograms, we'll look for the following:
...
* `ggplot(data, aes(x = BMI, fill = EDUC))`: Initializes the plot using `ggplot()` and specifies `data` as the dataset.
  * `aes(x = BMI, fill = EDUC)`: Defines the components to be mapped as follows:
...
welch_results <- data %>% 
  welch_anova_test(BMI_sqrt ~ EDUC)
...
5. **p.adj.signif** – An indicator of whether the test returned a significant result, where asterisks indicate statistical significance, and "ns" means "not significant."
...
This code chunk essentially recreates the boxplot figure we examined in our exploratory data analysis up above – but now, we're showing the square root-transformed BMI variable used in our analysis, and – based on the results of the Games-Howell test – we've added indicators of which pairwise comparisons were statistically significant.

<br>

# Summary

In Lab 6, we have learned key steps for carrying out the Analysis of Variance (ANOVA) procedure to investigate differences in means across more than two groups. Any analysis such as this should follow this general process: 
...
2. Check ANOVA assumptions, including normality and homogeneity of variance (or homoscedasticity) using statistical tests or visual approaches, as appropriate.
