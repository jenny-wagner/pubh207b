...
In Lab 3, we will apply estimation techniques for four population parameters: a single population mean, the difference between two population means, a single population proportion, and the difference between two population proportions. 
...
In Lab 3, we will continue working with the Framingham Heart Study teaching dataset. A key assumption we will make is that the baseline sample is *representative* of the adult population of the city of Framingham, MA.  
...
2. Next, save the **modified dataset** you created in Lab 2 - called "frmgham2_p1_impute.csv" - in the same folder as your Lab 3 R Markdown file.
...
In Lab 3, we will use functions from a few new packages, so these will need to be installed before you proceed. Copy and paste the following into your Console (bottom-left window) to install the packages: 
