...
2. Next, move or save the **modified dataset** you created in Lab 1 — called "frmgham2_p1.csv" — in the same folder as your Lab 2 R Markdown file.
...
Set your working directory manually (point-and-click method) or using the `setwd()` function — remember to paste the file path to the folder where you've saved your Lab 2 materials between the parentheses (return to Lab 1 and/or the RStudio Video Tutorial on Canvas if you need detailed instructions). 
...
Notice that, just like in Lab 1, using the `data <-` statement saves the data file as an object called 'data' in your Environment (top-right window).
...
This code creates a smaller dataset (called `data_sub`) containing only the variables of interest, listed above. Let's break down this line of code (from right to left):
...
* `data_sub <-`: Creates a new object called `data_sub` to store the result of the operation. By saving the subsetted data with a new name, the original dataset (called `data`) remains unchanged and available to us if we need it — for example, if we later decide we also want to look at diabetes diagnosis or some other variable(s).

After running the code chunk above, you should see a new object in your Environment (top-right window) called `data_sub` (when running your own analyses, you can use whatever names make sense to you) with 4,434 observations and 7 variables. Click on the `data_sub` object to view it in a new tab. You should see a spreadsheet open containing only the 7 variables we selected.

<br>

## 5. Identify and modify variable types

Like we did in Lab 1, we should identify variable types using the `class()` function and modify them as needed. Let's check the variable type for each variable in `data_sub`. We can use the code below to check the variable type for each variable in our dataset. Notice we're using a different approach than that shown in Lab 1 — the `sapply` function is essentially telling R to apply the function (`class`, in this case) to all columns in our dataset (`data_sub`). This is simply a more efficient way to check variable types; however, the approach shown in Lab 1 (applying the `class` function to each individual variable) is also perfectly appropriate.
...
We see that all variables are coded as "integer" variables aside from `BMI`, which is coded as "numeric". As discussed in Lab 1, we should modify variable types when needed to ensure variables are treated appropriately — as numerical or categorical — in our analyses. For example, we know the values for sex assigned at birth (`SEX`), attained education (`EDUC`), and obesity status (`OBESE`) represent categories rather than numerical quantities. As such, we should modify these variables accordingly using the `factor()` command (see Lab 1 for a detailed explanation of the code):
...
> You might be thinking, "Wait, didn't we already convert sex assigned at birth and attained education to factor variables in Lab 1? Why are they integer variables once again?" Note that when you first import a CSV file in a new R session, R will automatically interpret variables with numerical values as either "integer" or "numeric" — *R does not know that the values represent categories until we specify this.* So, any time you bring in a new CSV file, you will need to go through the process of identifying and modifying variable types as needed.
...
Our `data_sub` object contains three numerical variables (excluding participant ID number) — age (`AGE`), body mass index (`BMI`), and total cholesterol level (`TOTCHOL`) — and three categorical variables — sex assigned at birth (`SEX`), attained education (`EDUC`), and obesity status (`OBESE`). 
...
* **For numerical variables:**
  ...
  * Median: The middle value when data are sorted (i.e., the 50th percentile).
...
For example, we can see that the baseline sample contains 1,944 individuals assigned male sex at birth, where `SEX`=1, and 2,490 individuals assigned female sex at birth, where `SEX`=2. If you're thinking, "How do we know which category corresponds to each value?" Remember to check the data documentation, which tells us, for each variable, which category each value corresponds to. We can also see that the minimum and maximum ages in the sample are 32 and 70, respectively, with a mean of 49.93 years and median of 49 years. There are no missing values for sex or age. 

For practice, examine the output and consider the following:
...
> **What does `na.rm = TRUE` mean?** This argument is used to remove missing values (NA's) before performing a calculation. Some functions in R will automatically ignore missing values, while others require you to specify this explicitly. I know, kind of annoying. The `mean()` and `sd()` functions are two examples in which R will return an "NA" result in the presence of missing values — unless you explicitly tell R to ignore them. More on missing values later in this Lab.
...
Exploring the distribution of variables using descriptive plots complements summary statistics and frequencies by providing a visual representation of the data. While descriptive statistics and frequencies offer numerical summaries, plots can help us understand the overall shape of a distribution and reveal patterns, trends, and anomalies that the numbers alone may not capture. For example, histograms or boxplots of numerical variables can highlight skewness, outliers, or multimodal distributions, while bar charts of categorical variables can more easily show us the relative frequency of each category. Visualizing data can also help identify issues like possible data entry errors or unusual variability in addition to making it easier to communicate findings to others.

<br>

### Histograms

**Histograms** are useful visuals for *numerical* variables. A histogram shows the frequency of observations across "bins" of values and is best for assessing the shape and modality of a variable. For example, if a dataset includes ages ranging from 18 to 90, using bins of 5 years (e.g., 18–22, 23–27, etc.) might make sense to align with age groupings commonly used in research. This would require about 15 bins, where the frequencies generated would be the number of people with ages in each bin.
...
> Notice that the only changes we needed to make to the code to create these additional historgrams were to the specific variable of interest (specified in the first line of code in each chunk) and the labels (so that our x-axis and chart titles accurately reflect the variable at-hand). We also adjusted the number of bins here — but again, there is no single correct number of bins. 
...
Like histograms, boxplots are useful visuals for *numerical* variables. A boxplot provides a visual summary of key statistics (e.g., median, quartiles, range) and is *ideal for identifying outliers* and for comparing the distribution of a numerical variable across groups (more on this later).
...
* `boxplot(data_sub$AGE...)`: Creates a boxplot of the `AGE` variable from the `data_sub` object. The boxplot summarizes the distribution of ages, showing the median (dark line), interquartile range (gray box), and min/max values (the "whiskers"). Boxplots will also typically reveal any potential outliers, which will be represented by open circles, but we don't see any in this case.
...
After running the code chunk above, you should see a boxplot of age appear in the plots window (bottom-right). The distribution of age appears to be generally symmetrical (with mean equal to median) with no outliers (in alignment with our observations of the age histogram generated previously).
...
2. Determine **outlier thresholds**. The lower limit extends to the value Q1—1.5xIQR, and the upper limit extends to Q3+1.5xIQR. Points beyond these thresholds are considered outliers. The "whiskers" on a boxplot represent the most extreme observed values *within those thresholds*. In the absence of outliers, the whiskers represent the minimum and maximum values of the variable (as seen for age previously).
...
* `main = "Bar Chart of Sex Distribution"`: Sets the title of the chart.
* `xlab = "Sex"`: Labels the x-axis, indicating what the categories on the x-axis represent.
* `ylab = "Frequency"`: Labels the y-axis, indicating that the height of each bar represents the count of observations in each category.
* `col = "lightblue"`: Sets the color of the bars to light blue (totally optional — and fun to experiment with!).
...
After running the code chunk above, you should see a bar chart showing the distribution of sex in the plots window (bottom-right). Based on this chart, we can see that females make up a larger portion of the sample than males.

Two additional examples, for attained education and obesity status. Notice here how we don't need to change any of the commands to create bar charts for these additional variables — we only need to change the variable name, the chart title, the x-axis label, and specific category labels to align with the variable of interest:
...
Just like when we examined individual variables, we can use descriptive plots/visuals to explore relationships between variables. The method we use to explore relationships between variables depends on the *types of variables* involved, as different approaches are appropriate for different combinations of variable types. Here’s a brief overview with examples from our data...

<br>

### Two numerical variables

In general, we will use *scatterplots* to visualize the relationship between two numerical variables. For example, suppose we're interested in the relationship between BMI and total cholesterol level. On a scatterplot, one variable is plotted on the x-axis and the other on the y-axis. Scatterplots allow us to identify trends (e.g., positive, negative, or no correlation) and assess the linearity of the relationship.

```{r}
# Basic scatterplot of BMI and total cholesterol level
plot(data_sub$BMI, data_sub$TOTCHOL, 
     main = "Scatterplot of BMI vs. Total Cholesterol", 
     xlab = "BMI", 
     ylab = "Total Cholesterol Level")
```
...
* `plot(data_sub$BMI, data_sub$TOTCHOL...)`: Creates a scatterplot with BMI on the x-axis and total cholesterol level on the y-axis. Notice how for *both* BMI and total cholesterol level we are identifying the dataset for R to use (`data_sub`) and the specific variables of interest (`BMI` and `TOTCHOL`), separated by the `$` operator. Also notice that the variable we want on the x-axis is listed first, and the variable we want on the y-axis is listed second.
* `main = "Scatterplot of BMI vs. Total Cholesterol"`: Adds a title to the scatterplot.
* `xlab = "BMI"` and `ylab = "Total Cholesterol Level"`: Labels the x-axis and y-axis to make the plot easier to interpret.
...
* `boxplot(data_sub$BMI ~ data_sub$EDUC`...): Specifies the formula for the boxplot, where `BMI` is the numerical variable to be plotted, and `EDUC` is the categorical variable used to group the data. The two variables are separated by a tilde (`~`).
...
To explore the relationship between two categorical variables, we can create a *contingency table* to show the frequencies of combinations of the categories. We can then use a *bar chart* to visualize the frequencies (or proportions) within each category. For example, you might be interested in the relationship between sex assigned at birth and obesity status. Contingency tables and bar charts can help us determine whether certain categories are more or less likely to occur together. Here's an example:

```{r}
# Create a contingency table for obesity status by sex
...
```{r}
# Bar chart for obesity status by sex assigned at birth
barplot(table(data_sub$SEX, data_sub$OBESE), 
        beside = TRUE, 
        main = "Bar Chart of Obesity Status by Sex", 
        xlab = "Obesity Status", 
        ylab = "Frequency", 
...
When comparing frequencies or counts across groups, be aware that the sizes of the groups may differ, and this can distort your interpretation. For example, if there are more females than males in the sample, it might appear that females are more frequently obese simply because there are more females overall, not because they are more likely to be obese. To account for these differences, it’s important to examine *relative frequencies* or proportions, which standardize the data by dividing the count in each category by the total for that group. This approach allows for fair comparisons across groups of different sizes and provides a more accurate picture of the relationships in the data.

First, we can calculate **relative frequencies** in a few steps as follows:
...
Let's break down each piece of code in the chunk above:

* `contingency_table <- table(data_sub$SEX, data_sub$OBESE)`
  * `table(data_sub$SEX, data_sub$OBESE)`: Creates the contingency table for sex and obesity status.
  * `contingency_table <-`: Saves the resulting contingency table as an object called "contingency_table", which allows us to input this object directly into the subsequent function.
* `relative_frequencies <- prop.table(contingency_table, margin = 1)`:
  * `prop.table(contingency_table, margin = 1)`: Calculates relative frequencies for each group. The margin = 1 argument ensures that the proportions are calculated row-wise (i.e., within each sex category).
  * `relative_frequencies <-`: Saves the resulting relative frequencies as an object called "relative_frequencies", which allows us to input this object directly into the subsequent function.
...
Next, now that we've created an object called `relative_frequencies` (take a look at your Environment window to confirm) containing the proportion of males and females who are obese vs. not obese, we can plot these relative frequencies in a bar chart as follows:
...
From the bar chart of relative frequencies, we can see that females in the sample are slightly more likely to be obese than males. In the coming weeks in this course, we will learn how to test whether group differences like this are *statistically significant*, meaning there is enough evidence in the data to conclude that a real difference exists (in the population) — or in other words, that the difference we observe here is unlikely to have happened by chance.

> A key point... Understanding how variables relate to one another is a central part of public health research. Examining these relationships helps us understand how health outcomes, behaviors, and risk factors are connected, which in turn can inform effective interventions and policies. For example, exploring how smoking relates to lung health, how BMI is associated with blood pressure, or how educational attainment is linked to access to care can reveal important patterns in population health. At this stage, we are just beginning to explore these relationships using Exploratory Data Analysis (EDA). Later in the course, we will build on this foundation by learning more advanced analytic methods that allow us to formally answer research questions, such as identifying risk factors for disease or examining health disparities.
...
For some variables, we see that the list of summary statistics includes the number of `NA` values. NA means "not available" and indicates a missing data point. For example, of our 4,434 total observations, there are 19 missing values for BMI (and for obesity status, given this variable was derived from BMI), 52 missing values for total cholesterol level, and 113 missing values for attained education.  
...
We can use the `gg_miss_upset()` function from the *naniar* package we installed at the beginning of this Lab to create an upset chart as follows:
...
From the chart, we see that most of the missingness is isolated to individual variables. For example, there are 111 observations for which `EDUC` is the *only* missing variable, and 50 observations for which `TOTCHOL` is the *only* missing variable. Observations rarely have co-occurring missing values. For example, only one individual in the sample is missing values for BMI, obesity status, *and* total cholesterol.

In real life, our investigation of missing data would typically go a bit further — we would want to know, for example, whether there are patterns of missingness related to the actual *values* of other variables (e.g., do missing values of `TOTCHOL` tend to cluster within participants with high BMI?). While we won’t dive into this level of detail in this course, it’s important to understand that examining missing data can be a complex and time-consuming process, especially when working with large datasets.
...
Under this approach, rows in which data are missing for any variables involved in the analysis are excluded. In other words, if a participant is missing a value for *any* variable used in our analysis, that participant's data is removed from the analytic sample entirely. This approach is straightforward but may reduce the sample size significantly, especially if many variables have missing values. This approach is most appropriate when the missingness is minimal and occurs randomly. We can omit rows with missing data using `na.omit()` function as follows:
...
The `na.omit()` function tells R to remove all rows which contain any missing values. It is a good idea to save the modified dataset as a new object in our Environment, as we have done here (`data_complete`), so that we retain the original dataset for further use if needed. Notice that, after running the above code chunk, we have a new object in our Environment called `data_complete` which contains 4,253 observations and 7 variables (the same 7 variables we had in `data_sub`). By eliminating rows with missing data, we have eliminated 181 observations and retained approximately 96% of our original sample. Not too bad. However, if you, as the researcher, decide you do not want to eliminate these observations completely, simple imputation (or a more advanced method) may be a more appropriate strategy for handling missing data. Let's take a look at this approach next...

<br>

#### Simple imputation

Simple imputation entails replacing missing values with a single value, such as the mean, median, or mode of the variable. This method is simple and retains the sample size; however, it may underestimate variability in the data. As with complete case analysis, this approach is generally appropriate for variables with few missing values and no strong patterns of missingness. Let's consider when it might be appropriate to apply simple imputation using the mean, median, or mode as the replacement value. But first, let's duplicate our dataset by creating an object with a new name (`data_impute`) so that we can retain the original if needed...
...
* `data_impute$BMI`: Refers to the `BMI` column in the `data_impute` object. 
...
* `mean(data_impute$BMI, na.rm = TRUE)`: Calculates the mean (average) of the `BMI` column, excluding missing values. The `na.rm = TRUE` argument tells R to ignore NA's when calculating the mean.
...
Finally, **mode imputation** is appropriate for categorical variables, where missing values are replaced with the most frequently occurring category. For instance, it could be used for a variable like attained education, where missing values might be replaced with the most common category ("0-11 years"). We can use the following two steps to replace missing values of attained education with its mode:
...
* First line: Identifies the mode (most frequently occurring value/category) for the `EDUC` variable and assigns it to the object `mode_value`. More specifically...
  ...
  * `mode_value <-`: Assigns the mode (e.g., "0-11 years") to the object `mode_value` (you should see it appear in the Environment window).
