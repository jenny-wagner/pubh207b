...
As we have discussed in class, public health researchers often seek to understand relationships between variables — for example, to identify risk factors for disease and populations at increased risk. In Lab 6, we used Analysis of Variance (ANOVA) to test for differences in the mean of a continuous variable across three or more groups, or equivalently, to examine the relationship between a categorical independent variable (with 3+ levels) and a continuous dependent variable. In Lab 7, we shift our focus to relationships between two continuous variables. Correlation and simple linear regression are two techniques used to explore and quantify these relationships. Correlation measures the strength and direction of a linear association, while simple linear regression goes a step further by modeling how a dependent variable (outcome) changes in response to an independent variable (exposure).

**Correlation** is used when the goal is to determine whether two continuous variables are linearly related and to quantify the strength and direction of that relationship. There are various methods for calculating what is called a *correlation coefficient*. We will use the most common method, Pearson’s correlation coefficient (denoted by *r*). This coefficient ranges from -1 to 1, where values close to -1 or 1 indicate a strong linear relationship (either negative or positive) and values near 0 suggest little or no linear association. Remember, of course, that correlation does not imply causation — it only describes an association between variables.

**Simple linear regression**, on the other hand, models the relationship between an independent variable (predictor or exposure) and a dependent variable (outcome). It not only tells us whether a relationship exists but also provides a mathematical equation to describe the relationship and make predictions. This method fits a regression line to the data, represented by the equation...
...
This model can help answer questions like: What is the association between air pollution levels and hospital admission rates for respiratory diseases? or What is the association between physical activity level and blood pressure?
...
As we saw in our exploration of Analysis of Variance (ANOVA) last week, linear regression too has several assumptions which must be *reasonably* met in order for any conclusions we draw from a linear model to be valid. Briefly, these assumptions are as follows:
...
In previous Labs, we have used the `class()` function to check variable types. We can also use the `str()` command to check the structure of our dataset, including variable types, as follows: 
...
Though the code above should look very familiar from previous Labs, notice that we're using a new option here called "labels" – as you might suspect, this option allows us to specify labels corresponding to each level of our categorical variable. Here, we are telling R that the level "0" should be labeled with "Low SES", and the level "1" should be labeled with "High SES". These labels can be very helpful when creating plots or carrying out future analyses, particularly when working with multiple categorical variables with the same levels (as we have seen in the past!). 

<br>

## 5. Exploratory data analysis

Since we are working with a new dataset in this Lab, we need to start by doing some exploratory data analysis before jumping into more in-depth analyses. We should first have a good understanding of the distribution of each variable we plan to use. For this, we may want to generate both numerical summaries and descriptive plots. We can also begin to explore possible relationships between variables – for example, using scatterplots – in order to inform future analyses. 
...
* **Residuals**: Provides a quick summary of our model residuals. Residuals are the differences between the observed values of the outcome and the values predicted by the regression model for each observation.
* **Coefficients**: The coefficients table provides estimates for the intercept (β~0~) and slope (β~1~) of the simple linear regression equation, and it also indicates whether an independent variable (in this case we only have one, vaccination rate) is a statistically significant predictor of the dependent variable. Specifically, we have found:
  * **β~0~** (y-intercept): Technically speaking, the value 241.1388 means that when the vaccination rate is 0%, the predicted measles incidence is approximately 241 cases per 100,000 population. However, because a 0% vaccination rate falls outside the range of the observed data (the lowest vaccination rate is about 70%), this interpretation is primarily theoretical here.
...
Before relying on our regression results, we need to ensure that the model reasonably meets the assumptions required for valid statistical inference. Violations of these assumptions can lead to biased or misleading results. In this next section, we will assess model diagnostics (i.e., check assumptions) by examining model **residuals**, which we can use to check for homoscedasticity, normality, and potential outliers. These checks will help confirm whether our model is appropriate or if adjustments are needed before drawing final conclusions.
...
**Note:** The '1' in the code above refers to a specific plot to generate from the `plot()` command, in this case, the residuals vs. fitted values plot. Other values (e.g., 2, 3) generate different diagnostic plots. We'll look at some others later.
...
Just remember, the Shapiro-Wilk test is highly sensitive with large samples, meaning even small (potentially meaningless) deviations from normality may be detected. As such, when n>100, we will generally rely on visual methods to assess the normality assumption. 
...
Now that we have checked the assumptions of linear regression — and found that our model indeed reasonably meets all assumptions — we can interpret and use the regression equation to understand the relationship between vaccination rate and measles incidence. Let's quickly revisit the summary of our model:
...
Last semester, you were introduced to the concept of confounding — a common issue in observational research that can lead to misleading conclusions if not properly accounted for. In public health, we often want to understand the relationship between an exposure and an outcome, but relationships are rarely this simple in real life. Other variables may distort relationships, for example, by making them appear stronger, weaker, or even reversing the direction.
...
Let's compute VIF scores for each variable in our model to identify any potential multicollinearity issues. We can do this using the `check_collinearity()` function from the `performance` package:
...
So far in this Lab, we have assumed that each predictor variable in our model has an independent effect on the outcome. However, in many situations, the effect of one variable may depend on the level of another variable — this is known as an interaction effect (or effect modification). We might be interested in investigating interactions when we suspect (based on theory or prior research) that the relationship between an independent variable and the outcome of interest differs across levels of another variable. For example, we found earlier in this Lab that socioeconomic status does not act as a confounder of the relationship between vaccination rate and measles incidence. But suppose we suspect that the effect of vaccination rate on measles incidence *depends* on socioeconomic status. Suppose we think that, in lower SES areas, vaccination rates may need to be higher in order to achieve the same reduction in measles cases (perhaps due to overcrowding, more limited healthcare access, or other factors).
...
Note that when we include an interaction term, the *main effects* of each variable and *interaction effect* are all reported in the output. The interaction effect is shown with the two variables separated by a colon (:). Based on the p-values, however, we see that the main effect of SES category and interaction between vaccination rate and SES category are insignificant. Womp womp. Had the interaction term been significant, recall from lecture that:
...
* A negative (and significant) interaction coefficient suggests the effect of one predictor variable on the outcome is reduced by the presence of the other variable.
