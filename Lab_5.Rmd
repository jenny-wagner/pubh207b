...
In Lab 4 last week, we explored estimation — the first major area of statistical inference — including how to calculate confidence intervals for population parameters often of interest in public health, such as the population mean, difference between two means, population proportion, and difference between two proportions. Confidence intervals provide a range of plausible values for a parameter, which allows us quantify uncertainty in our estimates.
...
For Lab 5, you should not need to install any new packages. As usual, however, we need to start by loading necessary packages with the `library()` function, as follows:
...
Just as we did in Labs 2 and 4, we'll need to modify the variable types for sex assigned at birth, attained education, and obesity status. While these variables take on numerical values in our dataset, these values do not represent actual quantities — rather, they represent categories with no inherent quantity. Remember that we also need to specify whether a categorical or factor variable is ordered or unordered (i.e., ordinal or nominal). Notice in the code chunks below that, for *unordered* (i.e., nominal) categorical variables like sex assigned at birth and obesity status, we specify `ordered = FALSE`, whereas, for *ordered* (i.e., ordinal) categorical variables like attained education, we specify `ordered = TRUE`.
...
From the summary of our dataset, we can see the mean BMI in the sample is 25.85. This aligns with our view of the histogram for BMI, which shows the bulk of the distribution centered around this value. Suppose you want to know whether the population of Framingham has an average BMI that is *different* from the national average (at the time of data collection), 25. Using our sample data, we can conduct a hypothesis test for the value of the population mean to determine — with a specified level of significance — whether the population mean of BMI in Framingham is different from the national average. 
...
Recall from lecture that, after specifying the null and alternative hypotheses, our next step is to identify the appropriate **reference distribution** — i.e., the probability distribution that describes how our test statistic behaves *under the assumption that the null hypothesis is true.* When conducting a hypothesis test for the value of a single population mean, we will use either the *Z* or *t*-distribution. Just like when we constructed confidence intervals in Lab 4, the appropriate reference distribution depends on the following conditions:
...
* **p-value**: The p-value represents the probability of obtaining a test statistic as extreme as (or more extreme than) the one observed, *assuming that the null hypothesis (𝐻~0~) is true.* In other words, the p-value tells us how likely our sample results would be if there were actually no effect or difference in the population (i.e., if 𝐻~0~ were correct). In this case, the p-value is very, very small (essentially 0), indicating an extremely low probability that we would observe a sample mean this far from 25 *if the null hypothesis were true.* 
...
Remember, the null hypothesis (𝐻~0~) always represents the status quo or no effect/difference. In this case, the status quo is that the mean BMI in Framingham is not greater than 25 — meaning it could be equal to or less than 25. The alternative hypothesis (𝐻~𝑎~) represents the claim we want to test. Since we are specifically interested in whether the mean BMI is greater than 25, we use a greater than (>) sign in 𝐻~𝑎~.
...
* Just as in our previous example, we have a test statistic (t) and p-value — and, from the tiny p-value, you probably suspect (correctly) that we will again reject the null hypothesis.
...
Again building from Lab 4, suppose we next want to know whether mean BMI differs by sex assigned at birth — in other words, we want to know whether there is a statistically significant difference in population mean BMI between those assigned male and female sex at birth. 

First, let's visualize the distribution of BMI by sex in our sample: 

```{r}
# create side-by-side histograms to show the distribution of BMI by sex at birth
data %>%
...
**𝐻~0~:** There is no difference in mean BMI between males and females in the population.  
...
```{r}
# two-sample t-test for difference in mean BMI by sex at birth
t.test(BMI ~ SEX, 
       data = data, 
       var.equal = FALSE, 
       alternative = "two.sided", 
...
* `mu = 0`: Tests whether the difference in means is equal to 0 (i.e., no difference between groups).
...
In Lab 4, we estimated the proportion of the adult population in Framingham with obesity (defined as BMI>30). Now, we will take this a step further by conducting a one-sample hypothesis test for a population proportion to determine whether the prevalence of obesity in Framingham at baseline differs significantly from the national obesity prevalence at the time of data collection, which was about 13% (recall that the Framingham Heart Study started in 1948 − national obesity prevalence has tripled since then). This test will allow us to assess whether the observed obesity prevalence in Framingham is statistically different from the national prevalence or if any difference could be due to random variation in our sample.
...
From the output, we can see there are 574 individuals classified as obese, out of a total sample of 4,434. We can now enter all of the values into the `prop.test()` function to carry out this test, as follows:
...
Finally, suppose we want to test whether there is a statistically significant difference in obesity prevalence between those assigned male and female sex at birth in Framingham. We can state the null and alternative hypotheses as follows:
...
As when we generated a confidence interval for the difference in proportions in Lab 4, we first need to know the number of obese individuals by sex assigned at birth in the sample. We also need the total number of males and females in the sample. We can obtain these values using the 'table' command as follows:

```{r}
# generate counts of obesity and totals by sex assigned at birth
table(data$SEX, data$OBESE) # counts of obesity by sex at birth
table(data$SEX) # total by sex at birth
```

The code chunks above generate two tables in the Console window. The first provides a count of obese individuals by sex assigned at birth, and the second provides the totals by sex. From the first table, we see there are 232 males with obesity, and there are 342 females with obesity in the sample. From the second table, we see there are a total of 1,944 males in the sample and 2,490 females. Next, we will use these values to test whether there is a statistically significant difference in population proportions using the `prop.test()` function as follows:

```{r}
# two-sample hypothesis test for difference in obesity prevalence by sex assigned at birth
prop.test(c(232, 342), c(1944, 2490), 
          alternative = "two.sided", 
          conf.level = 0.95, 
          correct = FALSE)
```

Once again, this two-sample test examines whether obesity prevalence differs between males and females in the Framingham population. As the p-value (0.07633) is greater than 0.05, we *fail to reject the null hypothesis* — meaning we cannot conclude there is a statistically significant difference in obesity prevalence between males (11.93%) and females (13.73%) in the population.

This result aligns with our findings from Lab 4, where we constructed a confidence interval for the difference in population proportions. As we can see again in the output here, the 95% confidence interval (-0.0378, 0.0018) includes zero, which reinforces our conclusion that the true difference in obesity prevalence between males and females could be zero (i.e., no difference). The results of our hypothesis test confirm our previous conclusion that any observed difference is likely due to random variation (i.e., chance).

<br>

# Summary

In Lab 5, we applied hypothesis testing to assess differences in population means and proportions using data from the Framingham Heart Study. We conducted:
...
* A two-sample proportion test to test for differences in obesity prevalence by sex assigned at birth in Framingham.

These methods are fundamental tools in public health research, as they allow us to:

1. Evaluate health disparities (e.g., differences in obesity burden across populations).
2. Assess intervention effectiveness (e.g., does a new program reduce smoking prevalence in a population?).
